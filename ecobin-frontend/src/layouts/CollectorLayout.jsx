...
import {
 LayoutDashboard, FileText, Trash2, Star,
...
export default function CollectorLayout({ handleLogout, theme, toggleTheme }) {
 ...
 // Define navigation items now that notifications state is available
 const navItems = [
 { to: "/collector/dashboard", icon: LayoutDashboard, label: "Dashboard" },
 { to: "/collector/requests", icon: FileText, label: "Assigned Requests" },
 { to: "/collector/bins", icon: Trash2, label: "Assigned Bins" },
 { to: "/collector/recycling", icon: Star, label: "Recycling Submission" },
 { to: "/collector/notifications", icon: Bell, label: "Notifications", badge: notifications.filter(n => !n.is_read).length },
 { to: "/collector/profile", icon: Settings, label: "Profile & Settings" },
 ];
 ...
 className="hidden lg:flex text-muted hover:text-secondary dark:hover:text-white transition-colors"
 >
 <Menu size={22} />
 </button>
 <button 
 onClick={() => setSidebarOpen(true)} 
 className="lg:hidden text-muted hover:text-secondary dark:hover:text-white transition-colors"
 >
 <Menu size={22} />
 </button>
 <span className="font-display font-bold text-primary dark:text-emerald-400 hidden sm:block">Collector Portal</span>
 </div>
 
 <div className="flex items-center gap-4">
 <button 
 onClick={toggleTheme}
 className="p-1.5 rounded border border-gray-200 dark:border-slate-600 text-muted hover:text-secondary dark:hover:text-white transition-all text-sm cursor-pointer"
 >
 {isDark ? '☀️' : '🌙'}
 </button>
 {/* Notification Bell */}
 <div className="relative inline-block">
 <button onClick={() => setShowNotif(!showNotif)} className="relative text-muted hover:text-secondary dark:hover:text-white">
 <Bell size={20} />
 ...
 <div className="p-2 text-sm text-muted">No notifications</div>
 ) : (
 notifications.map((n) => (
 ...
 <img src={logo} alt="EcoBin Logo" className="h-9 w-9 object-contain" />
 <span className="text-sm font-medium text-secondary hidden sm:block">{displayName}</span>
 </div>
 );
 })()}
 <button
